...
def save_notepad_backup(window, counter):
    try:
        ...
        # Especifica la ruta donde quieres guardar los respaldos
        backup_folder = r"C:\Users\TecnoDX\Downloads\RESPALDAR NOTAS"
        os.makedirs(backup_folder, exist_ok=True)  # Crear la carpeta si no existe
        file_name = os.path.join(backup_folder, f"Backup_{window_title}_{timestamp}.txt")
